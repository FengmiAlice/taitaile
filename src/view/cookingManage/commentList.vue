    ...
    export default {
        ...
        methods: {
            fetchData() {
                ...
                this.$get(`/blog/admin/getListForB`, newObj).then(data => {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        if(this.list.length !==0){
                            this.$post('/blog/admin/deleteComment', {ids: Array.from(new Set(this.list))}).then(data => {
                                if (data.code === 0) {
                                    this.fetchData()
                                }
                            })
            ...
            remove(id) {
                this.$Modal.confirm({
                    ...
                    content: '<p>确认删除?</p>',
                    onOk: () => {
                        this.$post(`/blog/admin/${id}/deleteComment`).then(() => {
                                this.fetchData()
                            }
                        );
                    },
