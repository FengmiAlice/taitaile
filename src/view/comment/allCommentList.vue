<template>
    <Card @keydown.enter.native="handleSearch">
        <div class="search-con search-con-top">
            <DatePicker type="daterange" split-panels placeholder="发表时间" style="width: 200px" @on-change="changeDate"
            ...
            <Input clearable placeholder="评论内容" class="search-input"
    ...
    export default {
            ...
            return {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            return h('div', [
                                params.row.commentType === 'blog' ? '厨艺秀' : params.row.commentType==='answer'? '回答':'二级评论']
                            )
                        }
        ...
        methods: {
            fetchData() {
                ...
                this.$get(`/Comment/admin/getListForB`, newObj).then(data => {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        if(this.list.length !==0) {
                            this.$post('/Comment/admin/deleteComment', {ids: Array.from(new Set(this.list))}).then(data => {
                                if (data.code === 0) {
                                    this.fetchData()
                                }
                            })
            ...
            remove(id) {
                this.$Modal.confirm({
                    ...
                    content: '<p>确认删除?</p>',
                    onOk: () => {
                        this.$post(`/Comment/admin/${id}/delete`).then(() => {
                                this.fetchData()
                            }
                        );
                    },
